...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        x = self.layer1.forward(x).relu() # Pass through the first layer and apply ReLU activation
        x = self.layer2.forward(x).relu() # Pass through the second layer and apply ReLU activation
        return self.layer3.forward(x).sigmoid() # Pass through the third layer and apply Sigmoid activation for output
